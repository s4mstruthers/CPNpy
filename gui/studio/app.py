...
from PySide6.QtWidgets import (
    ...
    QFormLayout, QFrame, QGridLayout, QInputDialog, QLineEdit, QMainWindow, QMenu, QMessageBox,
...
class StudioWindow(QMainWindow):
    ...
    def _build_welcome(self) -> QWidget:
        ...
        cards = [
            ...
            ("Open a Petri net", "PNML from ProM, PM4Py, WoPeD… Check soundness, play the "
             "token game, explore the state space.", "Open PNML…", self.action_open),
    ...
    def _build_menus(self) -> None:
        ...
        self._action(file_menu, "New Coloured Petri Net", QKeySequence.New, self.action_new_cpn)
    ...
    def add_document(self, document, options: dict | None = None) -> None:
        ...
        if isinstance(document, LogDocument):
            ...
            page.inspector_tabs.changed.connect(
                lambda i: self.remembered.__setitem__("cpn_inspector", i))
    ...
    def open_path(self, path: str, csv_mapping: dict | None = None) -> None:
        ...
        try:
            if lower.endswith(".pnml"):
                net = read_pnml(path)
                net.name = Path(path).stem
                self.add_document(ModelDocument(net, path=path, origin="PNML file"))
                ...
                self.statusBar().showMessage(
                    f"Opened {name}" + (f" — {problems} problem(s), see the Problems tab"
                                        if problems else
                                        " — hard to read? Edit ▸ ✦ Tidy layout arranges it "
                                        "automatically"), 10000)
