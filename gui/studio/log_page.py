...
from ...mining.footprint import CAUSAL, INVERSE, PARALLEL, footprint_of_log
...
class LogPage(QWidget):
    ...
    def _build_footprint(self, layout) -> None:
        ...
        table.setHorizontalHeaderLabels(activities)
        table.setVerticalHeaderLabels(activities)
        table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        table.setSelectionMode(QAbstractItemView.NoSelection)
        table.horizontalHeader().setDefaultSectionSize(max(64, min(140, 900 // max(len(activities), 1))))
        table.verticalHeader().setDefaultSectionSize(34)
        table.horizontalHeader().setDefaultAlignment(Qt.AlignCenter)
        t = style.tokens()
        tint = {CAUSAL: style.qc(style.categorical(0), 0.16), INVERSE: style.qc(style.categorical(0), 0.08),
                PARALLEL: style.qc(style.categorical(1), 0.18)}
        big = theme.ui_font(15)
