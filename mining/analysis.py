...
def check_soundness(net: PetriNet, max_states: int = 200_000) -> SoundnessReport:
    ...
    if not workflow.is_workflow_net:
        report.findings += [f"Not a WF-net: {p}." for p in workflow.problems]
        return report
    ...
    if graph.has_omega:
        ...
        return report
    if graph.truncated:
        report.findings.append(f"State space exceeds {max_states} markings; soundness "
    ...
    if improper:
        ...
        report.findings.append(
            ...
            "while other tokens were left behind.")
    ...
    if dead:
        report.findings.append("Dead transitions (can never fire): "
